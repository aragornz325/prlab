...
''',
      ...
      if (query.isEmpty) {
        throw Excepciones.noEncontrado();
      }
  ...
  /// Modifica un registro de [Marca]. Requiere el ID de la [Marca]
  ...
  Future<bool> eliminarMarcaBorradoFisico({
    ...
    try {
      await ejecutarOperacionOrm(
        session,
        (session) async => await session.db.transaction(
          (transaction) async {
            await MarcaStaff.delete(
              session,
  ...
  Future<bool> eliminarMarca({
    ...
    try {
      ...
      var now = DatabasePoolManager.encoder.convert(
        DateTime.now(),
      );
...
UPDATE marcas_staff 
SET "ultimaModificacion" = $now, "fechaEliminacion" = $now 
...
''');
  ...
  Future<bool> asignarUsuarioAMarca(
    ...
    try {
      await ejecutarOperacionOrm(
        session,
        (session) async => await session.db.transaction((transaction) async {
          ...
          await ejecutarOperacionOrm(
            session,
            (session) async => await MarcaStaff.insert(session, marcaStaff),
          );
        }),
      );
